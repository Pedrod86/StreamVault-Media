...
export default function EmbyHomeRows() {
  const [playingItem, setPlayingItem] = useState(null);
  const [browsingItem, setBrowsingItem] = useState(null);

  const { data, isLoading } = useQuery({
    queryKey: ['embyHomeRows'],
    queryFn: () => base44.functions.invoke('embyHomeRows', {}).then(r => r.data),
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
  });
  ...
  const handlePlay = (item) => {
    if (item.type === 'Series') {
      setBrowsingItem({ ...item, embyId: item.id });
    } else {
      setPlayingItem(item);
    }
  };
  ...
  return (
    <>
      <EmbyRow title="Recently Added Movies" items={rows.recentMovies} onPlay={handlePlay} />
      <EmbyRow title="Recently Added TV Shows" items={rows.recentShows} onPlay={handlePlay} />
      <EmbyRow title="4K Movies" items={rows.fourKMovies} onPlay={handlePlay} />
      <EmbyRow title="4K TV Shows" items={rows.fourKShows} onPlay={handlePlay} />
      <EmbyRow title="Kids TV" items={rows.kidsShows} onPlay={handlePlay} />

      {playingItem && server && (
        <ExoPlayer
          src={`${server.server_url}/Videos/${playingItem.id}/stream?api_key=${server.api_token}&Static=true`}
