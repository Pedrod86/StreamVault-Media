...
function RecentCard({ item, onNavigate }) {
  const handleClick = () => {
    // Navigate straight to the live Jellyfin item — no DB lookup needed.
...
export default function JellyfinRecentlyAdded() {
  const navigate = useNavigate();

  const { data, isLoading, error } = useQuery({
    queryKey: ['jellyfinRecentlyAdded'],
    queryFn: async () => {
      const res = await base44.functions.invoke('jellyfinRecentlyAdded', {});
